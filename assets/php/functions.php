...
$response['status']=true;
    ...
    if(!$form_data['email']){
        $response['msg']="gmail is required";
        $response['status']=false;
...
function validateLoginForm($form_data){
    ...
    $blank=false;
        ...
        if(!$blank && !checkUser($form_data)['status']){
            $response['msg']="wrong username/email or password";
            $response['status']=false;
...
//for filtering the sugeestion list.........
...
//for checking the user is followed by current users or not...
...
//for getting users for follow suggestion 
...
function validateUpdateForm($form_data, $image_data){
    ...
    $response['status']=true;
        ...
        if($image_data['name']){
            ...
            if($size>4000){
                $response['msg']="upload image less than 4 mb";
                $response['status']=false;
                $response['field']='profile_pic';
            }
        }
...
function updateProfile($data,$imagedata){
    ...
    $username = mysqli_real_escape_string($db,$data['username']);
    $password = mysqli_real_escape_string($db,$data['password']);
    //$profile_pic = mysqli_real_escape_string($db,$data['profile_pic']);
